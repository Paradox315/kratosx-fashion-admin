export default {
  'login.form.title': '登录 Arco Design Pro',
  'login.form.username.errMsg': '用户名不能为空',
  'login.form.password.errMsg': '密码不能为空',
  'login.form.captcha.errMsg': '验证码不能为空',
  'login.form.email.errMsg': '邮箱格式不正确',
  'login.form.mobile.errMsg': '手机号格式不正确',
  'login.form.login.errMsg': '登录出错，轻刷新重试',
  'login.form.login.success': '欢迎使用',
  'login.form.register.success': '注册成功',
  'login.form.userName.placeholder': '用户名：admin',
  'login.form.password.placeholder': '密码：admin',
  'login.form.captcha.placeholder': '请输入验证码',
  'login.form.captcha.alt': '验证码',
  'login.form.rememberPassword': '记住密码',
  'login.form.forgetPassword': '忘记密码',
  'login.form.login': '登录',
  'login.form.register': '注册账号',
  'login.form.register.username': '用户名',
  'login.form.register.username.extra':
    '用户名只能包含字母、数字、下划线，且必须以字母开头',
  'login.form.register.password': '密码',
  'login.form.register.password.extra': '密码长度至少为8位',
  'login.form.register.mobile': '手机号',
  'login.form.register.email': '邮箱',
  'login.form.register.captcha': '验证码',
  'login.banner.slogan1': '开箱即用的高质量模板',
  'login.banner.subSlogan1': '丰富的的页面模板，覆盖大多数典型业务场景',
  'login.banner.slogan2': '内置了常见问题的解决方案',
  'login.banner.subSlogan2': '国际化，路由配置，状态管理应有尽有',
  'login.banner.slogan3': '接入可视化增强工具AUX',
  'login.banner.subSlogan3': '实现灵活的区块式开发',
};
